...
class TensorRTS_GymEnv(gym.Env, Tensor_Base):
    ...
    def step(self, action):
        ...
        reward = self.calculate_reward(win, lost, previous_power, current_power, action)
    ...
    def calculate_reward(self, win, lost, previous_power, current_power, action):
        reward = 0
        # Constants for reward tuning
        WIN_REWARD = 100
        LOSS_PENALTY = -100
        POWER_GAIN_REWARD = 1  # Reward for each unit of power gained
        RETREAT_REWARD = 5  # Reward for retreating strategically
        ...
        # Reward for power gain
        power_gain = current_power - previous_power
        if power_gain > 0:
            reward += power_gain * POWER_GAIN_REWARD

        # Defensive strategy reward
        if action == 1:  # Assuming action 1 is retreat
...
if __name__ == "__main__":  # This is to train
    ...
    for i in range(episodes):
        ...
        print("win count: ", env.win_count)
        print("loss count: ", env.loss_count)  
        print("win rate: ", env.win_count / (env.win_count + env.loss_count)) 
