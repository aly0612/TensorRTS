...
class NeutronBot(Agent):
  def __init__(self, initial_observation: Observation, action_space: Dict[ActionName, ActionSpace], script_dir : str, bot_dir="checkpoint"):
    super().__init__(initial_observation, action_space, script_dir)
    checkpoint = load_checkpoint(os.path.join(script_dir, "checkpoint"))
  ...
  def on_game_start(self, is_player_one:bool, is_player_two:bool) -> None:
    ...
    checkpoint = load_checkpoint(os.path.join(self.script_dir, "checkpoint"))
    self.agent = RogueNetAgent(checkpoint.state.agent)
